...
#         markets_ICO = ["AUGUR", "ark","neo"]
    ...
    def saveStatsToExcel(self, mkt_stat):
        ...
        file_path = self.fileName(
            "Market Stats " + datetime.datetime.now().strftime("%d. %B %Y %I:%M%p"), "xlsx")
...
#        mkt_stat.describe(percentiles=[.05,.1,.2,.3,.4,.5,.6,.7,.8,.9,.95])[[0,1,4,5]].to_excel(writer,'Stats', startrow=len(mkt_stat)+2, index=True)
    ...
    def calculateStats(self, p_market_list, p_exchange, p_interval):
        ...
        for market_name in p_market_list:
            ...
            try:
                print(str(market_count) + " of " + str(total_markets) + " " +
                      mkt.getMarketName() + ", " + mkt.getInterval() + " on " + mkt.getExchange())
                ...
                err_msg = market_name + " does not exist. " + \
                    str(sys.exc_info()[0])
                new_exception.raiseException(self.__class__.__name__,
                ...
                new_exception = ExceptionSkyzeAbstract()
                err_msg = "ERROR: unknown - " + str(sys.exc_info()[0])
                new_exception.raiseException(self.__class__.__name__,
                                             inspect.currentframe().f_lineno,
                                             err_msg,
                                             self._logger,
...
#                 print(mstats.toDataFrame())
    ...
    def backtestIndependent(self, p_title, p_initial_equity, p_market_strategies, p_start_date, p_end_date, p_scope="all"):
        ...
        backtester = BackTester(title, p_initial_equity,
                                backtest_markets, p_start_date, p_end_date)
