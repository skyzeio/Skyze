...
class Market():
  ...
  def __init__(self,
    ...
    # Reverse if Required
    if p_reversed:
      self.market_data = p_market_data.sort_index(ascending=True, axis=0)
    else:
  ...
  def constructFileName(self, p_testing):
    ...
    if p_testing:
      file_name = os.path.join(
          settings_skyze.test_data_file_path, "%s.csv" % self.market_name)
    else:
  ...
  def readMarketDataCSV(self, p_market="NOTPASSED", p_testing=False):
    ...
    try:
      ...
      print(
          "FileNotFound - Market::readMarketDataCSV .... IOError File does not exist")
      raise IOError(
          "FileNotFound Raised - Market::readMarketDataCSV .... IOError File does not exist\nFile Path:   " + file_path)
      return
    except:
