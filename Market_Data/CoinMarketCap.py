...
#             mkt_data = self.scrapeHistoricalData(p_market_details["id"].tolist())
        ...
        p_markets.to_csv(file_path,header=False, date_format='%Y-%m-%d %H:%M:%S')
        return
...
#                 error_list = [market] + error_list
                ...
                print("UNKNOWN ERROR: CoinMarketCap::updateMarketData ... market_data = self.scrapeHistoricalData")
...
#                     no_data_list = no_data_list.append(market)
                    ...
                    print("Number of rows imported: "+str(len(market_data)-1))
                    
                    
                    payload = pd.DataFrame(market_data, columns = ["Date","Open","High","Low","Close","Volume","MC"]) 
                    if len(payload) > 0: 
                        payload.Date = pd.to_datetime(payload.Date, format="%Y%m%d")     # convert to datetime
#                         payload.Date2 = payload.Date[0].strftime('%Y-%m-%d %H:%M:%S')
#                         pd.DatetimeIndex(payload[0]*10**9)                # all_markets.json() is class 'dict'
                        payload = payload.set_index('Date')               # Set the Timestamp as the indes
